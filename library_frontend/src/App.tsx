...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="max-w-5xl space-y-8">
      ...
      {/* ── Stats Cards ── */}
      <div>
        <h2 style={{fontFamily:'Playfair Display, serif'}} className="text-xl font-semibold text-[#1a1209] mb-3">
          Overview
        </h2>
        <div className="grid grid-cols-4 gap-4">
          {stats.map(({ icon, label, value, color, to }) => (
            <div key={label}
              ...
              {loading
                ? <div className="w-8 h-7 bg-[#ede5d0] rounded animate-pulse mt-1" />
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {quickActions.map(({ icon, label, to }) => (
            <button key={label}
              onClick={() => navigate(to)}
              className="flex items-center justify-center gap-3 px-4 py-3 bg-[#1a1209] text-yellow-400 rounded-lg border border-yellow-700 hover:bg-yellow-900/40 hover:border-yellow-500 transition-all duration-200 font-semibold text-sm shadow-sm">
        ...
        <div className="bg-white rounded-lg border border-[#cfc4aa] shadow-sm overflow-hidden">
          {loading ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#1a1209] text-yellow-400 text-left">
              ...
              <tbody>
                {recentLoans.map((loan, i) => (
                  <tr key={loan.id}
                    className={`border-t border-[#ede5d0] hover:bg-[#faf7f2] transition-colors ${i % 2 === 0 ? 'bg-white' : 'bg-[#fdf9f4]'}`}>
